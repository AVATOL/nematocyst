...
namespace HCSearch
{
	...
	RankFeatures StandardPruneFeatures::computeFeatures(ImgFeatures& X, ImgLabeling& Y, set<int> action)
	{
		...
		phi.segment(numClasses*unaryFeatDim + numPairs*pairwiseFeatDim, numPairs*mutexFeatDim) = mutexTerm;
	...
	int StandardPruneFeatures::featureSize(ImgFeatures& X, ImgLabeling& Y, set<int> action)
	{
		...
		return numClasses*unaryFeatDim + numPairs*pairwiseFeatDim + numPairs*mutexFeatDim;
	}
